...
`dicom-skill` is an agent-ready shell skill for DICOM DIMSE workflows. It gives an agent a small, auditable command-line toolkit for verifying DICOM nodes, querying metadata, retrieving studies, anonymizing local DICOM payloads, and sending DICOM files between PACS, VNA or other DIMSE-compatible systems.
...
- The anonymizer removes metadata PHI according to the bundled script, but it does not perform OCR or burned-in pixel PHI removal. Review modality-specific risks before sharing images externally.
...
├── resources/
│   └── rsna/
│       ├── default-anonymizer.script
│       ├── LICENSE-RSNA-anonymizer.txt
│       └── README.md
└── scripts/
...
The DIMSE and anonymization commands only require Python dependencies. Docker is only needed for
...
### 7. Anonymize local DICOM files

Use `scripts/dicom_anonymize.py` only on local files or folders. It does not contact
remote DICOM nodes. The default workflow uses the bundled
...
```text
anonymized/study/<AnonPatientID>/<AnonStudyUID>/<AnonSeriesUID>/<AnonSOPUID>.dcm
```

For production use, provide your own registered UID root or keep the default
`2.25` deterministic UUID-style OID mode. A project salt can be supplied directly
...
The default patient ID strategy is sequential within the run, similar to the
RSNA anonymizer model. Use `--map-json secure/anon_map.json` to reuse mappings
across runs. That mapping file can contain PHI because original identifiers are
used as keys, so it must be stored securely. Use `--patient-id-strategy hashed`
when a deterministic patient pseudonym is preferred and the project has an
appropriate salt.

The anonymizer marks datasets with `PatientIdentityRemoved=YES` and adds
`DeidentificationMethod` / `DeidentificationMethodCodeSequence`. It does not
remove burned-in annotations or perform OCR-based pixel PHI removal.
...
- Explicit output directories such as `downloads/`, `anonymized/`, or `audit/` for retrieved
  payloads, anonymized payloads, and command results.
...
For a local smoke flow, see [`examples/orthanc-local.md`](examples/orthanc-local.md). For a local anonymization example, see [`examples/anonymize-local.md`](examples/anonymize-local.md).

## RSNA anonymizer note

Nota: a parte de anonimização baseada no RSNA foi feita a partir do script https://github.com/RSNA/anonymizer. O material copiado do projeto RSNA está em `resources/rsna/` e acompanha a licença Apache 2.0 original em `resources/rsna/LICENSE-RSNA-anonymizer.txt`.
