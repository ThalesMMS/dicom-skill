---
name: dicom-skill
description: Perform DICOM DIMSE C-ECHO, C-FIND, C-GET/C-MOVE retrieval, and C-STORE send operations from an agent shell. Includes helper scripts to create a temporary Orthanc receiver with AE title AGENT on DICOM port 4242 when a C-MOVE destination is needed.
---

# dicom-skill

Use this skill when the user asks an agent to interact with DICOM nodes/PACS/VNA/Orthanc using DIMSE operations: verify connectivity, query metadata, retrieve instances, or send DICOM files.

This is a shell/CLI skill, not an MCP server. Do not start an MCP server. Use the scripts in `scripts/` directly.

## Safety and data handling

DICOM metadata and files can contain patient-identifying information. Do not connect to clinical systems, retrieve studies, or send images unless the user has authorization and has provided the DICOM endpoint details. Keep outputs in local, user-controlled folders. Avoid printing full patient data unless the user explicitly asks for it. Do not anonymize by assumption; anonymization is a separate task.
...
The DIMSE scripts require Python packages only. The temporary Orthanc helper requires Docker and the `orthancteam/orthanc` image, pulled automatically by Docker if absent.
...
6. For send, verify the destination with C-ECHO, run a dry-run on the file set, then send.
7. Save full command output JSON with `--out-json` for auditability and use `--summary` for terminal/chat output on large studies.
8. After successful verification, clear temporary ZIP/DICOM payload files created in the workspace while preserving audit JSON/logs by default.
